...
namespace Jyy201831980105.Models
{
    public class chengji
    {
        ...
        //[RegularExpression(@"^[\w\u4e00-\u9fa5\s]*$", ErrorMessage = "请输入仅由字母和数字组成的产品名称。")]
        [RegularExpression(@"^[\u4e00-\u9fa5\a-zA-Z0-9]*$", ErrorMessage = "请输入由【中文、字母、数字或 - 、或 .】组成的课程名称。")]
        [DataType(DataType.Text)]
        public string Xueshengmingcheng { get; set; }

        [Display(Name = "课程成绩")]
        [Required(ErrorMessage = "【课程名称】不能为空白")]
        [StringLength(2, MinimumLength = 1, ErrorMessage = "请输入 2 字以内的【课程名称】。")]
        //[RegularExpression(@"^[\w\u4e00-\u9fa5\s]*$", ErrorMessage = "请输入仅由字母和数字组成的产品名称。")]
        [RegularExpression(@"^[0-9.-]*$", ErrorMessage = "请输入由【数值】组成的课程成绩。")]
        [DataType(DataType.Text)]
